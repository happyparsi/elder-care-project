...
// HealthCoach Component (unchanged)
...
// Login Component (improved with better layout and demo styling)
const Login = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      const backendError = error.response?.data?.error || 'Network/DB issue';
      alert(`Login failed: ${backendError}. Try rebuilding Docker (down -v && up --build).`);
  ...
  return (
    <div className="login-container">
      ...
      <div className="demo-credentials">
        <strong>Demo credentials (password: pass for all):</strong><br/>
...
// PatientDashboard (center-aligned via dashboard class)
...
// CaregiverDashboard (center-aligned via dashboard class)
...
// FamilyDashboard (center-aligned via dashboard class)
...
// AdminDashboard (updated with separate admin-section wrappers for cleaner layout)
const AdminDashboard = () => {
  ...
  const [newFamily, setNewFamily] = useState({ name: '', contact: '', assignedPatients: '' });
  ...
  const fetchAllData = async () => {
    try {
      ...
      const [reportsRes, patientsRes, caregiversRes, familyRes, appointmentsRes, medicationsRes] = await Promise.all([
        ...
        axios.get('http://localhost:3001/family').catch(() => ({ data: [] })),
    ...
    <div className="dashboard">
      ...
      <section className="admin-section">
        ...
        <form onSubmit={handleAddPatient} className="form">
          ...
          <label>
            Contact:
            <input type="text" value={newPatient.contact} onChange={(e) => setNewPatient({...newPatient, contact: e.target.value})} required />
          </label>
          <label>
            Medical History:
            <textarea value={newPatient.medicalHistory} onChange={(e) => setNewPatient({...newPatient, medicalHistory: e.target.value})} rows="3" />
          </label>
        ...
        <ul className="list">
          {patients.map((p, index) => (
            <li key={p.PatientID} style={{ animationDelay: `${index * 0.1}s` }}>
              <span><strong>{p.Name}</strong> - Contact: {p.Contact}<br />Medical History: {p.MedicalHistory}</span>
      ...
      <section className="admin-section">
        ...
        <form onSubmit={handleAddCaregiver} className="form">
          <label>
            Name:
            <input type="text" value={newCaregiver.name} onChange={(e) => setNewCaregiver({...newCaregiver, name: e.target.value})} required />
          </label>
          <label>
            Contact:
            <input type="text" value={newCaregiver.contact} onChange={(e) => setNewCaregiver({...newCaregiver, contact: e.target.value})} required />
          </label>
          <label>
            Assigned Patients (comma-separated IDs):
            <input type="text" value={newCaregiver.assignedPatients} onChange={(e) => setNewCaregiver({...newCaregiver, assignedPatients: e.target.value})} required />
          </label>
        ...
        <ul className="list">
          {caregivers.map((c, index) => (
            <li key={c.CaregiverID} style={{ animationDelay: `${index * 0.1}s` }}>
              <span>{c.Name} - {c.Contact} - Assigned: {c.AssignedPatients}</span>
      ...
      <section className="admin-section">
        ...
        <form onSubmit={handleAddFamily} className="form">
          <label>
            Name:
            <input type="text" value={newFamily.name} onChange={(e) => setNewFamily({...newFamily, name: e.target.value})} required />
          </label>
          <label>
            Contact:
            <input type="text" value={newFamily.contact} onChange={(e) => setNewFamily({...newFamily, contact: e.target.value})} required />
          </label>
          <label>
            Assigned Patients (comma-separated IDs):
            <input type="text" value={newFamily.assignedPatients} onChange={(e) => setNewFamily({...newFamily, assignedPatients: e.target.value})} required />
          </label>
        ...
        <ul className="list">
          {family.map((f, index) => (
            <li key={f.FamilyID} style={{ animationDelay: `${index * 0.1}s` }}>
              <span>{f.Name} - {f.Contact} - Assigned: {f.AssignedPatients}</span>
      ...
      <section className="admin-section">
        ...
        <form onSubmit={handleAddAppointment} className="form">
          <label>
            Patient ID:
            <input type="number" value={newAppointment.patientId} onChange={(e) => setNewAppointment({...newAppointment, patientId: e.target.value})} required />
          </label>
          <label>
            Caregiver ID:
            <input type="number" value={newAppointment.caregiverId} onChange={(e) => setNewAppointment({...newAppointment, caregiverId: e.target.value})} required />
          </label>
          <label>
            DateTime (YYYY-MM-DD HH:MM:SS):
            <input type="text" value={newAppointment.dateTime} onChange={(e) => setNewAppointment({...newAppointment, dateTime: e.target.value})} required />
          </label>
          <label>
            Description:
            <input type="text" value={newAppointment.description} onChange={(e) => setNewAppointment({...newAppointment, description: e.target.value})} required />
          </label>
        ...
        <ul className="list">
          {appointments.map((a, index) => (
            <li key={a.ApptID} style={{ animationDelay: `${index * 0.1}s` }}>
              <span>Patient {a.PatientID} with Caregiver {a.CaregiverID} on {a.DateTime}: {a.Description}</span>
      ...
      <section className="admin-section">
        ...
        <form onSubmit={handleAddMedication} className="form">
          <label>
            Patient ID:
            <input type="number" value={newMedication.patientId} onChange={(e) => setNewMedication({...newMedication, patientId: e.target.value})} required />
          </label>
          <label>
            Drug Name:
            <input type="text" value={newMedication.drugName} onChange={(e) => setNewMedication({...newMedication, drugName: e.target.value})} required />
          </label>
          <label>
            Dosage:
            <input type="text" value={newMedication.dosage} onChange={(e) => setNewMedication({...newMedication, dosage: e.target.value})} required />
          </label>
          <label>
            Time Schedule:
            <input type="text" value={newMedication.timeSchedule} onChange={(e) => setNewMedication({...newMedication, timeSchedule: e.target.value})} required />
          </label>
        ...
        <ul className="list">
          {medications.map((m, index) => (
            <li key={m.MedID} style={{ animationDelay: `${index * 0.1}s` }}>
              <span>{m.DrugName} ({m.Dosage}) for Patient {m.PatientID} at {m.TimeSchedule}</span>
